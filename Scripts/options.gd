...
@export var popup_panel_number : PopupPanel
@export var popup_panel_duplicate : PopupPanel
...
func _process(_delta):
		...
		if popup_panel_number.visible:
			popup_panel_number.max_size = get_viewport_rect().size
		if popup_panel_duplicate.visible:
			popup_panel_duplicate.max_size = get_viewport_rect().size
...
func _on_validate_button_pressed():
	...
	var children = peopleContainer.get_children()
	if len(children) < 3:
		popup_panel_number.visible = true
		return
	var res = check_duplicate(children)
	if res[0]:
		popup_panel_duplicate.visible = true
		children[res[1]].select_all()
		children[res[2]].select_all()
		return
...
func _on_popup_number_ok_button_pressed():
	popup_panel_number.visible = false

func _on_popup_duplicate_ok_button_pressed():
	popup_panel_duplicate.visible = false
